...
namespace BL.Database.Common
{
    internal static class CommonQueries
    {
        ...
        {
            var dbContext = context.DbContext as DmsContext;
            var qry0 = dbContext.DocumentEventsSet.Where(x => x.ClientId == context.Client.Id)
            ...
            var taOld = dbContext.DocumentTaskAccessesSet.Where(x => x.ClientId == context.Client.Id).Where(x => x.Task.DocumentId == documentId)
...

            {
                ...
                dbContext.DocumentTaskAccessesSet.RemoveRange(dbContext.DocumentTaskAccessesSet.Where(x => x.ClientId == context.Client.Id).Where(filterContains));
            }
