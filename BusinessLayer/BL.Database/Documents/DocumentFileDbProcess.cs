...
namespace BL.Database.Documents
{
    public class DocumentFileDbProcess : CoreDb.CoreDb, IDocumentFileDbProcess
    {
        public IEnumerable<DocumentAttachedFile> GetDocumentFiles(IContext ctx, int documentId)
        {
            ...
            return
                sq.Join(dbContext.DocumentFilesSet, sub=>new { sub.DocId, sub.OrdId, VerId = sub.MaxVers}, fl=> new { DocId = fl.DocumentId, OrdId = fl.OrderNumber, VerId = fl.Version}, (s,f)=> new {fl = f})
                .Join(dbContext.DictionaryAgentsSet, df => df.fl.LastChangeUserId, da => da.Id, (d, a) => new { d.fl, agName = a.Name })
        ...
        public DocumentAttachedFile GetDocumentFileVersion(IContext ctx, int documentId, int orderNumber, int versionNumber)
        {
            ...
            return
                dbContext.DocumentFilesSet
                .Where(x => x.DocumentId == documentId && x.Version == versionNumber && x.OrderNumber == orderNumber)
                .Join(dbContext.DictionaryAgentsSet, df => df.LastChangeUserId, da => da.Id, (d, a) => new { fl = d, agName = a.Name })
                .Select(x => new DocumentAttachedFile
                {
                    Id = x.fl.Id,
        ...
        public void UpdateFileOrVersion(IContext ctx, DocumentAttachedFile docFile)
        {
            ...
            var fl =
                dbContext.DocumentFilesSet.FirstOrDefault(x => x.Id == docFile.Id ||
                        (x.DocumentId == docFile.DocumentId && x.OrderNumber == docFile.OrderInDocument &&
