    ...
    public static class CommonDocumentUtilities
    {
        ...
        public static void SetSendListAtrributesForNewDocument(IContext context, IEnumerable<InternalDocumentSendList> sendLists, bool? isInitial)
        {
            foreach (var sl in sendLists)
            {
                ...
                var sourceAccess = sl.AccessGroups.FirstOrDefault(y => y.AccessType == EnumEventAccessTypes.Source);
                if (sourceAccess != null)
                {
                    sourceAccess.PositionId = context.CurrentPositionId;
                }
                else
                {
        ...
        public static IEnumerable<InternalDocumentAccess> GetNewDocumentAccesses(IContext context, IEnumerable<InternalDocumentEventAccess> accesses, EnumAccessLevels accessLevel = EnumAccessLevels.PersonallyAndIOAndReferents)
        {
                                                                            ...
                                                                            List<AccessGroup> accessGroups = null)
        {
            return new List<InternalDocumentEvent>
            {
                GetNewDocumentEvent(context,entityTypeId, documentId,eventType,eventDate,description,addDescription,parentEventId,taskId,accessGroups),
            };
        }
        ...
        public static InternalDocumentSubscription GetNewDocumentSubscription(IContext context, InternalDocumentSendList sendListModel, EnumEventTypes? eventType = null, EnumEventCorrespondentType? eventCorrespondentType = null, bool? isAddTargetCopy = null)
        {
            ...
            var res = new InternalDocumentSubscription
            {
                ...
                SendEvent = eventType == null ? null :
                            GetNewDocumentEvent(context, sendListModel.EntityTypeId, sendListModel.DocumentId, eventType.Value, null, sendListModel.Description, null, null, sendListModel.TaskId, accessess)
            };
            SetLastChange(context, res);
            return res;
        }
        ...
        public static InternalDocumentEvent GetNewDocumentPaperEvent(IContext context, int entityTypeId, int documentId, int? paperId, EnumEventTypes eventType, string description = null, int? targetPositionId = null, int? targetAgentId = null, int? sourcePositionId = null, int? sourceAgentId = null, bool IsMarkPlan = true, bool IsMarkRecieve = true)
        {
            ...
            var res = new InternalDocumentEvent
            {
                ...
                CreateDate = DateTime.UtcNow,
                Description = description,
                TargetPositionId = targetPositionId ?? context.CurrentPositionId,   //TODO multi
        ...
        public static DmsExceptions VerifyAndSetDocumentAccess(IContext ctx, InternalDocument document, InternalDocumentEvent newEvent, VerifySubordination modelVerifySubordination = null, bool isThrowException = true, EnumAccessLevels accessLevel = EnumAccessLevels.PersonallyAndIOAndReferents)
        {
            var operationDb = DmsResolver.Current.Get<IDocumentOperationsDbProcess>();
            if (!document.IsRestrictedSendListsLoaded)
            {
                operationDb.SetRestrictedSendListsPrepare(ctx, document);
            }
            if (!VerifyAccessesByRestrictedSendLists(ctx, document.RestrictedSendLists, newEvent.Accesses, isThrowException)) return new DocumentSendListNotFoundInDocumentRestrictedSendList();
            var targetPosition = newEvent.Accesses.Where(x => x.AccessType != EnumEventAccessTypes.Source && x.PositionId.HasValue).Select(x => x.PositionId.Value).ToList();
            if (newEvent.ParentEventId.HasValue)
            {
                operationDb.SetParentEventAccessesPrepare(ctx, document, newEvent.ParentEventId.Value);
            ...
            document.Accesses = GetNewDocumentAccesses(ctx, newEvent.Accesses.Where(x => x.PositionId.HasValue && targetPosition.Contains(x.PositionId.Value)), accessLevel);
            return null;
        }
