    ...
    public class CancelPostponeDueDateDocumentCommand : BaseDocumentCommand
    {
            ...
            _actionRecords =
                _document.Waits.Where(
                    x =>
                        ...
                        x.OnEvent.EventType != EnumEventTypes.MarkExecution && x.OnEvent.EventType != EnumEventTypes.AskPostponeDueDate && x.IsHasAskPostponeDueDate && x.OnEvent.ControllerPositionId == positionId) &&
                        x.OffEventId == null &&
        ...
        public override object Execute()
        {
            ...
            var evAcceesses = (Model.TargetCopyAccessGroups?.Where(x => x.AccessType == EnumEventAccessTypes.TargetCopy) ?? new List<AccessGroup>())
                ...
                .ToList();
            var newEvent = _docWait.OffEvent = CommonDocumentUtilities.GetNewDocumentEvent(_context, (int)EnumEntytiTypes.Document, _docWait.DocumentId, EnumEventTypes.CancelPostponeDueDate, Model.EventDate, 
